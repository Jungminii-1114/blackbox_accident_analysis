...
첫 노트북 `01_stage3_can_lab.ipynb`은 model trainer가 아니라 **데이터 계약과 label audit을
통과해야만 학습으로 넘어가는 gate**다. 입력 manifest에 video path, frame timestamp, speed,
steering, route group가 없거나 시간이 단조롭지 않으면 실패한다. 그 뒤 route-group split,
threshold grid, class balance, fixed-class Macro-F1과 baseline comparison을 기록한다.

두 번째 노트북은 이 audit manifest를 입력으로 Stage 3 V1을 학습하며, 세 번째가 Stage 2
event proposal을 다룬다. 이 순서는 실행이 아니라 성공 확률의 순서다.
